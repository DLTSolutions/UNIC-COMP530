...
#Actual factorization takes place below
if composite:
    ...
    for j in range(3,sqrtnumber+1,2):
        #Check if the number j is a prime, else it may NOT be used for factorization
        sqrtj = math.ceil(math.sqrt(j)) #number must be rounded up to the next integer else in error
        for k in range(2, sqrtj+1):
            if (j % k) == 0: # If the Modulus operator returns 0 then it's NOT A PRIME so it may NOT be used for factorization
                print (j, "will not be used for factorization since it's not a prime") 
                break
            else:
